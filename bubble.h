...
const int BUBBLE_DEFAULT_VX = 50;
...
public:
    ...
    void nextStep(double t)
    {
        ...
        double new_y = circle.getY() + vy*t + 1000*t*t;
        ...
        if ((vx < 0 && new_x < (0 + circle.getRadius())) // bounce along X direction at left border
            ||
            (vx > 0 && new_x > (500 - circle.getRadius()))) // bounce along X direction at right border
        {
        ...
        if ((vy < 0 && new_y < (0 + circle.getRadius())) // bounce along Y direction at top border
            ||
            (vy > 0 && new_y > (500 - circle.getRadius() - 50))) // bounce along Y direction at bottom border
        {

            vy = -vy;
            new_y = circle.getY() + vy*t + 1000*t*t;
